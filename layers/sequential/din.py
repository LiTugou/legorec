...
#         """
#         key=sequence
#         query=tf.tile(query,[1,self.maxseqlen]) ## (bs,emb_dim) -> (bs,emb_dim*maxseqlen)
#         query=tf.reshape(query,[-1,self.maxseqlen,self.emb_dim]) # (bs,emb_dim*maxseqlen) -> (bs,maxseqlen,emb_dim)
#         ## key 和 query的一些交互,如果有其它特征other_emb 可以一并拼接
#         din_all=tf.concat([query,key,query-key,query*key],axis=-1) ## (bs,maxseqlen,emb_dim*4)
...
class DinAttention(layers.Layer):
    ...
    def call(self,sequence,query,seqlen):
        ...
        """
        key=sequence
        maxseqlen=tf.shape(key)[1]
        query=tf.tile(query,[1,maxseqlen]) ## (bs,emb_dim) -> (bs,emb_dim*maxseqlen)
        query=tf.reshape(query,[-1,maxseqlen,self.emb_dim]) # (bs,emb_dim*maxseqlen) -> (bs,maxseqlen,emb_dim)
        ## key 和 query的一些交互,如果有其它特征other_emb 可以一并拼接
        din_all=tf.concat([query,key,query-key,query*key],axis=-1) ## (bs,maxseqlen,emb_dim*4)
        ## 一个2层mlp (emb_dim*2,emb_dim,1)
        din_w=self.attlayer(din_all)
        din_w=tf.reshape(din_w,[-1,1,maxseqlen])
        ## where 返回bool，或者是满足条件时din_w[i]*paddings[i]
        ## 即din_w[seqlen+1:]=0
        seq_mask=tf.sequence_mask(seqlen, maxseqlen, dtype=tf.bool)
        seq_mask=tf.reshape(seq_mask,[-1,1,maxseqlen])
        paddings=tf.zeros_like(din_w)
        din_w = tf.where(seq_mask, din_w, paddings)
        ## (bs,1,maxseqlen) @ (bs,maxseqlen,emb_dim) -> (bs,1,emb_dim)
        return tf.reshape(tf.matmul(din_w, key), [-1, self.emb_dim])
